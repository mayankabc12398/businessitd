  ...
  return (
    <div className="page">
      ...
      {loading ? (
        ...
        <div className="wf-board" style={{ display: 'grid', gridTemplateColumns: `repeat(${WORKFLOW_STEPS.length}, minmax(190px, 1fr))`, gap: 12, overflowX: 'auto', paddingBottom: 8 }}>
          {WORKFLOW_STEPS.map((s) => {
            const items = byStage[s.step] || [];
            const Icon = STEP_ICON[s.icon] || FileText;
            return (
              <div key={s.key} className="card card-pad" style={{ background: 'var(--surface-2)', minWidth: 190 }}>
                ...
                <div className="flex-col gap-2">
                  {items.map((f) => (
                    <button key={f.id} className="card card-pad card-hover" style={{ padding: 10, background: 'var(--surface)', cursor: 'pointer', textAlign: 'left' }} onClick={() => navigate(`/revenue/features?id=${f.id}`)}>
                      <div className="fw-6 t-xs" style={{ lineHeight: 1.3 }}>{f.name}</div>
                      <div className="flex items-center justify-between mt-1"><span className="t-xs ink-3 truncate">{f.client}</span><Badge tone={PRIO_TONE[f.priority]}>{f.priority}</Badge></div>
                    </button>
                  ))}
