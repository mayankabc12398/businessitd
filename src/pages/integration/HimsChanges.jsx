...
export default function HimsChanges() {
  ...
  const dbRows = useMemo(() => [...dbExtra, ...(db || [])], [dbExtra, db]);
  ...
  const addDbChange = (v) => {
    ...
    setDbExtra((prev) => [{
      ...
      trigger: v.trigger || '—', scheduler: v.scheduler || '—', alterScript: v.alterScript || '',
    }, ...prev]);
    toast.success('Database change recorded', v.tableName);
    setShowDb(false);
  };
  ...
  return (
    <div className="page">
      <PageHeader icon={<Database size={22} />} tint="peach" title="HIMS & Database Changes" desc="Every screen, logic and database change made for an integration (Modules 5 & 6)"
        ...
        actions={tab === 'db'
          ? <button className="btn btn-primary" onClick={() => setShowDb(true)}><Plus size={15} /> Add Database Change</button>
          : <button className="btn btn-primary" onClick={() => setShow(true)}><Plus size={15} /> Add HIMS Change</button>} />

      <div className="kpi-grid stagger">
        <MetricCard label="HIMS Changes" value={himsRows.length} tint="peach" icon={<Code2 size={19} />} footer="Screen & logic changes" />
        <MetricCard label="Database Changes" value={dbRows.length} tint="lemon" icon={<TableProperties size={19} />} footer="Tables, SPs, triggers" />
      ...
      <div className="card card-pad" style={{ paddingBottom: 0 }}>
        <Tabs tabs={[{ key: 'screens', label: 'Screen & Logic Changes', count: himsRows.length }, { key: 'db', label: 'Database Changes', count: dbRows.length }]} active={tab} onChange={setTab} />
      </div>
      ...
      <FormDrawer open={show} onClose={() => setShow(false)} title="Record HIMS Change" subtitle="Document a screen / logic change (Module 5)"
        ...
        fields={[
          ...
          { name: 'apiCalled', label: 'API Called', type: 'search', options: apiOptions, placeholder: 'Select an API from the catalogue…' },
      ...
      <FormDrawer open={showDb} onClose={() => setShowDb(false)} title="Record Database Change" subtitle="Document a table / SP / trigger change (Module 6)"
        ...
        fields={[
          ...
          { name: 'modifiedSP', label: 'Modified Stored Procedures', full: true, placeholder: 'usp_GetInvoiceBalance' },
          { name: 'trigger', label: 'Trigger', placeholder: 'trg_MpesaLog_Audit' },
          { name: 'scheduler', label: 'Scheduler / Job', placeholder: 'job_MpesaReconcile (every 15 min)' },
          { name: 'alterScript', label: 'Alter Script', type: 'textarea', full: true, rows: 4, placeholder: 'ALTER TABLE PaymentTransaction ADD MpesaReceipt VARCHAR(20) NULL;' },
        ]} />
    </div>
  );
}
