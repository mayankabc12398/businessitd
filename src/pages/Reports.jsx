...
export default function Reports() {
  ...
  const { data: requirements } = useApi(() => api.getRequirements());
  const { data: masterData } = useApi(() => api.getMasterData());
  const { data: uatCases } = useApi(() => api.getUatCases());
  const { data: training } = useApi(() => api.getTraining());
  const { data: goliveReadiness } = useApi(() => api.getGoLiveReadiness());
  const { data: issues } = useApi(() => api.getIssues());
  const { data: risks } = useApi(() => api.getRisks());
  const { data: signoffs } = useApi(() => api.getSignoffs());
  ...
  const REPORTS = [
    ...
    { key: 'srs', title: 'SRS Schedule Report', desc: 'Department-wise SRS plan, status & sign-off', icon: FileSignature, tint: 'mint',
      rows: srsSchedule || [], file: 'srs-schedule.csv', cols: [
    ...
    { key: 'master', title: 'Master Data Status', desc: 'Requested / received / imported per master', icon: Database, tint: 'lemon',
      rows: masterData || [], file: 'master-data.csv', cols: [
    ...
    { key: 'uat', title: 'UAT Status Report', desc: 'Module coverage, pass/fail & sign-off', icon: FlaskConical, tint: 'lavender',
      rows: uatCases || [], file: 'uat-status.csv', cols: [
    ...
    { key: 'golive', title: 'Go-Live Readiness', desc: 'Readiness % & checklist per project', icon: Rocket, tint: 'cyan',
      rows: goliveReadiness || [], file: 'go-live-readiness.csv', cols: [
